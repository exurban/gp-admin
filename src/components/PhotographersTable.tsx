...
const PhotographersTable: React.FC<Props> = ({
  ...
  const [search, { data: searchData }] = useLazyQuery(SearchPhotographersDocument, {
    variables: {
      input: {
  ...
  const { error, loading, data } = useQuery(SearchPhotographersDocument, {
    variables: {
      input: {
        searchString: searchString
      }
  ...
  if (error) return <p>Error loading photographers</p>;
  if (loading) return <p>Loading...</p>;

  const photographers = searchData ? searchData.searchPhotographers : data?.searchPhotographers;
      ...
      client.cache.writeQuery({
        ...
        data: {
          searchPhotographers: {
            ...
            datalist: orderBy([...data.searchPhotographers.datalist], sortBy, dir)
          }
        },
        variables: {
          input: {
    ...
    <Flex marginTop="80px" marginX="auto" flexDirection="column" maxHeight="500px">
      ...
      >
        <Table isHoverable hasDividers overflow="unset">
          ...
          <Table.Body>
            {photographers?.datalist?.map(pg => (
              <Table.Row
                key={pg.id}
                // @ts-ignore
                onClick={() => handleRowClick(pg)}
                background={pg.id === selectedItem?.id ? "rgba(158, 70, 215, 0.7)" : undefined}
                color={pg.id === selectedItem?.id ? "white" : "text"}
              >
                <Table.Cell>{pg.name}</Table.Cell>
                <Table.Cell>{pg.email}</Table.Cell>
                <Table.Cell style={{ textAlign: "center" }}>{pg.countOfPhotos}</Table.Cell>
              </Table.Row>
            ))}
          </Table.Body>
        </Table>
