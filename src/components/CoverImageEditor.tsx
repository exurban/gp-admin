...
const CoverImageEditor: React.FC<Props> = forwardRef(
  ({ coverImage, setCoverImage, imageUrl, setImageUrl, name, closeModal }, ref) => {
    ...
    const [addImage] = useMutation(AddImageDocument, {
      onCompleted(data) {
        if (data.addImage.success) {
          data.addImage.newImage ? setCoverImage(data.addImage.newImage) : null;
          toasts.success({
            title: `Success`,
    ...
    const [updateImage] = useMutation(UpdateImageDocument, {
      onCompleted(data) {
        if (data.updateImage.success) {
          data.updateImage.updatedImage ? setCoverImage(data.updateImage.updatedImage) : null;
          toasts.success({
            title: `Success`,
    ...
    const addCoverImage = (input: AddImageInput) => {
      addImage({
        variables: {
          input
        }
    ...
    const updateCoverImage = (imageId: number, input: UpdateImageInput) => {
      updateImage({
        variables: {
          id: imageId,
          input
    ...
    const saveImageData = (url: string) => {
      ...
      if (!coverImage) {
        ...
        const imageId = parseInt(coverImage?.id);
        ...
        updateCoverImage(imageId, input);
      }
    };
