...
import { SearchTagsDocument, Tag } from "../graphql-operations";
import { Box, Flex, Icon, Button, Table, Text } from "bumbag";
import Search from "./Search";
import { orderBy } from "lodash";

type Props = {
  selectedItem: Tag | undefined;
...
const TagsTable: React.FC<Props> = ({
  ...
  const [search, { data: searchData }] = useLazyQuery(SearchTagsDocument, {
    variables: {
      input: {
  ...
  const { error, loading, data } = useQuery(SearchTagsDocument, {
    variables: {
      input: {
        searchString: searchString
      }
  ...
  const tags = searchData ? searchData.searchTags : data?.searchTags;
      ...
      client.cache.writeQuery({
        ...
        data: {
          searchTags: {
            ...
            datalist: orderBy([...data.searchTags.datalist], sortBy, dir)
          }
        },
        variables: {
          input: {
    ...
    <Flex marginTop="80px" marginX="auto" flexDirection="column" maxHeight="500px">
      ...
      >
        <Table isHoverable hasDividers overflow="unset">
          ...
          <Table.Body>
            {tags?.datalist?.map(tag => (
              <Table.Row
                key={tag.id}
                // @ts-ignore
                onClick={() => handleRowClick(tag)}
                background={tag.id === selectedItem?.id ? "rgba(158, 70, 215, 0.7)" : undefined}
                color={tag.id === selectedItem?.id ? "white" : "text"}
              >
                <Table.Cell>{tag.name}</Table.Cell>
                <Table.Cell style={{ textAlign: "center" }}>{tag.countOfPhotos}</Table.Cell>
              </Table.Row>
            ))}
          </Table.Body>
        </Table>
